...
13. **`match_backdrop.py`'s tight-crop patch dict is a hardcoded prefix list — a new garment's "-detail.jpg" falls through to the full floor-fit path and can wreck it.** (14 Sep, found on `short-wine-detail.jpg`/`short-blush-detail.jpg`.) The `tight` check only recognised filenames containing `tee-`, `loungeset-`, or `legging-`; `short-` was never added when the short SKU shipped. So `short-*-detail.jpg` (a hip/waistband crop with almost no real floor in frame) fell through to `fit_field`'s floor-line detection, which mis-detected a floor around y≈1865 in a crop that has none, then applied a smoothly-varying 2D backdrop-gain field across the whole frame — including the tee and garment, which got misclassified as "backdrop-like" by the per-pixel distance test. The result: a soft, cloud-shaped brightness pattern baked into the tee (looked like a glossy/latex sheen with a faint ghosted shape) and a visibly paler, greyer garment colour than every other view of the same colourway. Symptom to watch for: a tight `-detail.jpg` crop that reads paler/greyer than its own hero shot, or a plain tee with an odd soft blotchy sheen — check whether its prefix is in `match_backdrop.py`'s `patch` dict before assuming it's a Gemini rendering issue. Fix: add the new garment's filename prefix to BOTH the `tight` tuple and the `patch` dict (fractions of a clean backdrop patch actually visible in that crop — sampled and confirmed low-saturation/low-std before trusting it). Whenever a new garment's `-detail.jpg` is added to the pipeline, add its prefix to `match_backdrop.py` in the same commit — don't wait to notice it in review. Re-finishing from the `-raw.jpg` (crop → grade → backdrop-match) after the dict fix is the correct repair; patching the already-baked JPEG's pixels (colour-matching, deglare) is a workaround, not a fix, and won't be right pixel-for-pixel.

14. **`qc.py`'s "garment_matches" check only compared the garment NAME/colour, never actual construction — a jacket with no pockets passed twice.** (14 Sep, found by eye on the loungeset hero/back after the fit fix: the pocket seams rendered but no pocket opening/bag did, and QC still said PASS both times.) Cause: `run_qc.py` built the garment hint as `"<name> in <colour>"` (e.g. "Off-Duty Set in cream") — Gemini was only ever asked "does the garment in the image match this label", never shown the real product photo to check features against. Fix: `qc.py` now accepts `--garment-ref <path>` (the real CJ photo) and `--construction <text>` (the `line` description from `brand.json`); when given, it puts the real photo in front of Gemini as image 3 and asks it to compare feature-by-feature — collar, zip, seams, cuffs, and specifically pockets — returning `construction_accurate`/`construction_notes`, which fails the run if false. `run_qc.py` now passes both automatically for every job item that isn't a `detail` shot (close crops get the human-eyes gate instead, per rule 3). Lesson: a QC "PASS" that only checks name and colour is not checking the product — always re-verify a new construction-fidelity check like this one actually catches the defect that motivated it, on the same image that failed by eye, before trusting it on the next batch.
